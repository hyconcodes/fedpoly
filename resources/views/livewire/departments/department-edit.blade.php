<div class="relative mb-6 w-full">
    <flux:heading size="xl" class="my-4" level="1">{{ __('Edit Department') }}</flux:heading>
    @include('includes.messages')
    <flux:separator variant="subtle" />

    <form wire:submit="updateDepartment" class="max-w-2xl space-y-6 my-8">
        <!-- Department Name -->
        <flux:input
            wire:model="name"
        ...
        @else
            ...
            <p class="text-sm text-secondary-600 dark:text-secondary-400">{{ __('Loading schools or no schools available to assign.') }}</p>
        @endif
